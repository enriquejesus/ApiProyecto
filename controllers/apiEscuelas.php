<?php
    ...
    *  De la linea 11 - 21 contiene funciónes especificas para contener los mensajes
    ...
    */
    ...
    function delete($id){//Inició function delete
        $escuela = new EscuelasModel();//Se crea un objeto y se asigna a EscuelasModel();
        
        $res = $escuela->eliminarEscuela($id);//Se manda a llamar el query y se agrega el id 

        if($res->rowCount() > 0){//Verifica si trae un dato comparandolo si es mayor con 0
            $this->exito('Eliminado...');//Mensaje de confirmación
            
        }//fin del if
        else{//en caso de que no se confirme la parte del if
            //echo json_encode(array('mensaje' => 'No hay elementos'));
            $this->error('No hay elementos relacionados con la busqueda');//Si no se confirma la eliminación manda este mensaje
        }//
    }//fin de la función delete

    function update($item){//Inició function update
        $escuela = new EscuelasModel();//Se crea un objeto y se asigna a EscuelasModel();
    
        $res = $escuela->actualizaEscuela($item);//Se manda a llamar el query y se agrega el item
        $this->exito('Actualizo el registro de manera exitosa');//Mensaje de confirmación
